...
class Graph:
    ...
    def getVertices(self):
        ...
        return list(range(self._numVerts))  # made to be list(range()) for python 3 compatibility
...
class ListGraph(Graph):
    ...
    def __init__(self, n, nodeData = []):
        ...
        self._adjList = []
        for i in range(n): # keeping range the same from python 2 since it is only used to iterate
...
class MatrixGraph(Graph):
    ...
    def __init__(self, n, nodeData = []):
        ...
        self._adjMatrix = []
        row = [None] * n
        for i in range(n): # keeping range() since it is used as an iterable
    ...
    def getNeighbors(self, node):
        ...
        if node < self._numVerts:
            neighs = []
            for i in range(self._numVerts): #keeping range() since used as iterable
...
class WeightedMatrixGraph(MatrixGraph):
    ...
    def getNeighbors(self, node):
        ...
        if node < self._numVerts:
            neighs = []
            for i in range(self._numVerts): # keeping original range since used as iterable
