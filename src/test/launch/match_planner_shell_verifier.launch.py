...
def declare_launch_arguments():
    return [
        DeclareLaunchArgument("app_config", default_value="match_planner.yaml"),
        DeclareLaunchArgument(
            "verifier_config",
            default_value="match_planner_shell_verifier.yaml",
        ),
    ]
...
def generate_launch_description():
    ...
    # ---------------- Initialize Nodes ----------------
    # Start the Phase 4 planner shell and localizer stub.
    app_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(app_launch_file),
        launch_arguments={"config": app_config}.items(),
    )

    # Start the service/subscriber-based shell verifier.
    verifier = Node(
        package="test",
    ...
    ld.add_action(
        RegisterEventHandler(
            OnProcessExit(
                ...
                on_exit=[
                    EmitEvent(event=Shutdown(reason="match planner verifier finished")),
                ],
            )
        )
    )
