...
checkpoint_dir = str(checkPts) + f"/2026CellPredict_checkpoint-{run_id}/"
...
loaded_checkpoint_raw = "2026CellPredict_checkpoint-0622261052/TestHeadingInCellPredAdam224Corrected-10-1.26.keras"
...
def buildNetwork():
    ...
    if loaded_checkpoint is not None:
        ...
        model = create_pretrained_cnn_lstm(num_classes=outputSize, num_frames=seqLength, img_shape=(224, 224, 3))
...
def create_pretrained_cnn_lstm(num_classes, num_frames=10, img_shape=(224, 224, 3)):
    pretrained_cnn = tf.keras.applications.MobileNetV3Small(
        weights='imagenet',
        include_top=False, # remove the final classification head
        pooling='avg', # Automatically flattens the output to a 1D vector per frame
        input_shape=img_shape
    )

    # Freeze the pretrained weights so they do not update during initial training
    pretrained_cnn.trainable = False

    model = models.Sequential([
        layers.Input(shape=(num_frames, *img_shape)),
        layers.TimeDistributed(pretrained_cnn),
...
def fineTuneNetwork():
    """Unfreezes the base CNN layers for fine-tuning and recompiles the model."""
    global model

    # Access the inner MobileNet model
    for layer in model.layers:
        if "time_distributed" in layer.name.lower():
            inner_mobilenet = layer.layer

            # Unfreeze ONLY the last few layers (e.g., top 20 layers)
            # First, ensure individual layers can be modified
            for inner_layer in inner_mobilenet.layers:
                inner_layer.trainable = False  # Freeze everything first

            # Now unfreeze the last 20 layers for fine-tuning
            for inner_layer in inner_mobilenet.layers[-20:]:
                inner_layer.trainable = True
                print(f"Unfroze: {inner_layer.name}")

    # 3. CRITICAL STEP: Re-compile the outer model to apply the changes
    # Use a very low learning rate for fine-tuning to prevent destroying weights
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=1e-5),
        loss='categorical_crossentropy',  # Use your original loss function
...
if __name__ == "__main__":
    ...
    # 2a. Set up loaded from checkpoint model to finetune with all but last 20 layers
    fineTuneNetwork()
